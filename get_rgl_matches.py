...
def row_to_player(tr):  # type: (bs4.element.Tag) -> Optional[RglPlayer]
    ...
    """
    # cells = tr.find_all("td")bs4.element.Tag 
    ...
    player_dates = [i.text.strip() for i in date_rows if not i.find("a")] # type: List[str]
...
def row_to_match(tr):  # type: (bs4.element.Tag) -> Optional[RglMatch]
    ...
    team_links = tr.find_all("a", href=TEAM_RE) # type: Optional[bs4.element.ResultSet]
    date_cells = tr.find_all("td", text=RGL_DATE) # type: Optional[bs4.element.ResultSet]

    if not map_images or not team_links or not date_cells:
        return None
    ...
    two_number_cells = tr.find_all("td", text=RGL_SCORE) # type: Optional[bs4.element.ResultSet]
    if two_number_cells:
    ...
    if not match_date:
        return None
    match_dtime = datetime.strptime(match_date.strip(), "%m/%d/%Y %I:%M %p %Z")
...
for s in seasons:
    ...
    print(len(team_links), "team links found")

    for l in team_links:
        team_id_match = re.search(TEAM_RE, l.get("href"))
        if not team_id_match:
            print("no team id found for {}".format(s))
            continue
...
print(len(team_names), "team names found")
print(len(team_regions), "regions found")
...
for tid, rid in team_regions.items():
    ...
    if not league_link:
        print("no league link for team", tid, "skipping it")
        continue
    ...
    player_rows = [tr for tr in team_doc.find_all("tr")
                   if tr.find_all("a", href=PLAYER_RE)]
    ...
    match_rows = [tr for tr in team_doc.find_all("tr")
                  if tr.find_all("a", href=MATCH_RE)] # type: List[Optional[RglMatch]]
